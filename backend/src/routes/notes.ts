...
const include = { tags: { include: { tag: true } }, attachments: true, resources: true, author: { select: { name: true, email: true } } } as const;
const tagWrites = (tags: string[]) => ({ create: tags.map(name => ({ tag: { connectOrCreate: { where: { name }, create: { name } } } })) });
