...
describe("Notes API", () => {
  ...
  it("rejects an empty title", async () => { const response = await request(app).post("/api/notes").send({ title: "", content: "x" }); expect(response.status).toBe(400); });
  it("returns 404 when deleting a missing note", async () => { const response = await request(app).delete("/api/notes/missing"); expect(response.status).toBe(404); });
  it("returns 404 when deleting a missing attachment", async () => { const response = await request(app).delete("/api/notes/note_1/attachments/missing"); expect(response.status).toBe(404); });
});
